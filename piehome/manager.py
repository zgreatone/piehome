...
class SystemManager(object):
    ...
    def _support_action(self, device, action):
        ...
        if (action == controller.POWER_ON or action == controller.POWER_OFF) and \
                (1 in device.capabilities or 2 in device.capabilities):
            return True
        elif action == controller.BRIGHTNESS and 2 in device.capabilities:
            return True
        elif (action == controller.ARM or action == controller.DISARM) and 3 in device.capabilities:
            return True
